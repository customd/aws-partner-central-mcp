...
2. Cut a **GitHub Release** and **attach `dist/aws-partner-central.mcpb`** as a release asset. Tag the release to match the manifest `version` (currently `1.0.1`).
3. End users install by:
   ...
   - fill in the **configuration dialog**: AWS SSO start URL, 12-digit account ID, SSO role name, AWS region, default catalog (`AWS`/`Sandbox`), and log level, then click **Install**.
...
each GitHub Release to match (currently `1.0.1`). Claude Desktop uses the manifest
